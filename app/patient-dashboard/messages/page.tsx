...
export default function PatientMessagesPage() {
  ...
  const loadNames = async (messages: any[]) => {
    ...
    console.log('=== DEBUG: loadNames called ===');
    console.log('Messages:', messages);
    console.log('All IDs to load:', Array.from(allIds));
    console.log('Agency data:', agencyData);
    console.log('Staff table name:', agencyData.staff_table_name);
    console.log('Patients table name:', agencyData.patients_table_name);
    ...
    try {
      ...
      if (agencyData.staff_table_name && allIds.size > 0) {
        console.log('Querying staff table:', agencyData.staff_table_name);
        console.log('Querying for IDs:', Array.from(allIds));
      ...
      if (agencyData.patients_table_name && allIds.size > 0) {
        console.log('Querying patients table:', agencyData.patients_table_name);
        console.log('Querying for IDs:', Array.from(allIds));
      ...
      console.log('Final names object:', names);

      // Debug: Check if the specific missing ID exists in staff table
      const missingId = 'af9dbb1c-ca7d-4831-8dd4-cb68ad953e80';
      if (agencyData.staff_table_name && Array.from(allIds).includes(missingId)) {
        console.log('=== DEBUG: Checking missing ID in staff table ===');
        const { data: missingStaffData, error: missingStaffError } = await supabase
          .from(agencyData.staff_table_name)
          .select('id, first_name, last_name')
          .eq('id', missingId);
        
        console.log('Missing ID query result:', { data: missingStaffData, error: missingStaffError });
      }
  ...
  const loadMessages = async () => {
    if (!patientData?.agency_name || !agencyData) {
    ...
    try {
      ...
      if (agencyData.inbox_table_name) {
        const { data: inboxData, error: inboxError } = await supabase
          ...
          .select('*')
          .eq('recipient_id', patientData.id)
      ...
      console.log('=== DEBUG: About to call loadNames ===');
  ...
  const fetchPatientData = async (patientEmail: string) => {
    try {
      ...
      const { data: agencyData, error: agencyError } = await supabase
        .from('agency')
        .select('patients_table_name, staff_table_name, inbox_table_name, sent_table_name, drafts_table_name')
  ...
  // Load messages when tab changes or when agency data is available
  useEffect(() => {
  ...
  const loadAvailableRecipients = async (recipientType: string) => {
    ...
    try {
      ...
      if (recipientType === 'staff') {
        console.log('Looking up agency for staff table...');
        // Find the agency to get staff table name
        const { data: agency, error: agencyError } = await supabase
          .from('agency')
          .select('staff_table_name')
          .eq('agency_name', patientData.agency_name)
          .single();

        console.log('Agency lookup result:', { agency, agencyError });
        ...
        if (agencyError || !agency?.staff_table_name) {
          ...
          const { data: allAgencies } = await supabase.from('agency').select('agency_name, staff_table_name');
          console.log('All agencies:', allAgencies);
          return;
        }

        console.log('Loading staff from table:', agency.staff_table_name);
        // Load staff from the agency's staff table
        const { data: staffData, error: staffError } = await supabase
          .from(agency.staff_table_name)
          .select('id, first_name, last_name, role, department, email, status')
          .order('first_name');
        ...
        // Use all staff members (both online and offline)
        recipients = staffData || [];
        console.log('All staff recipients:', recipients);
      } else {
        console.log('Looking up agency for patients table...');
        ...
        if (agencyError || !agency?.patients_table_name) {
          ...
          console.log('Available agencies:');
          const { data: allAgencies } = await supabase.from('agency').select('agency_name, patients_table_name');
          console.log('All agencies:', allAgencies);
          return;
        }

        console.log('Loading patients from table:', agency.patients_table_name);
        // Load patients from the agency's patients table
        const { data: patientsData, error: patientsError } = await supabase
          .from(agency.patients_table_name)
          .select('id, first_name, last_name, medical_record_number, email, status')
          .order('first_name');

        console.log('Patients query result:', { patientsData, patientsError });
        ...
        // Use all patients (both online and offline)
        recipients = patientsData || [];
        console.log('All patient recipients:', recipients);
      }
      
      setAvailableRecipients(recipients);
      console.log('=== DEBUG: loadAvailableRecipients completed ===');
    } catch (error) {
  ...
  const handleFormChange = (field: string, value: string) => {
    ...
    if (field === 'recipientType') {
      ...
      // Clear recipient selection when type changes
      setNewMessageForm(prev => ({
        ...prev,
        recipient: ''
      }));
  ...
  const handleSendMessage = async () => {
    ...
    if (!patientData || !agencyData) {
      alert('Patient or agency data not available');
      return;
    }
    ...
    try {
      ...
      console.log('Patient ID (sender):', patientData.id);

      // Insert into sent table
      const sentData = {
        agency_name: patientData.agency_name,
        sender_id: patientData.id,
        sender_type: 'patient',
      ...
      console.log('Switching to sent tab to show the new message');
      setActiveTab('sent');
  ...
  const markMessageAsRead = async (messageId: string) => {
    ...
    try {
      // First, get the message details to find the corresponding sent message
      const { data: inboxMessage, error: fetchError } = await supabase
        .from(agencyData.inbox_table_name)
        .select('*')
        .eq('id', messageId)
        .single();
      ...
      if (inboxError) {
        console.error('Error marking inbox message as read:', inboxError);
        return;
      }

      // Update corresponding sent message with read_at timestamp
      if (agencyData.sent_table_name) {
        const { error: sentError } = await supabase
          .from(agencyData.sent_table_name)
          .update({ 
          ...
          .eq('sender_id', inboxMessage.sender_id)
          .eq('sender_type', inboxMessage.sender_type)
          .eq('recipient_id', inboxMessage.recipient_id)
          .eq('recipient_type', inboxMessage.recipient_type)
          .eq('subject', inboxMessage.subject)
          .eq('message_content', inboxMessage.message_content);

        if (sentError) {
          console.error('Error updating sent message read_at:', sentError);
        } else {
          console.log('Sent message read_at updated successfully');
        }
      }
  ...
  const handleTabChange = (tabId: string) => {
    ...
    if (tabId === 'inbox') {
      ...
      setMessages(sentMessages);
      // Debug: Check if sent messages have the right data
      console.log('=== DEBUG: Tab switched to sent ===');
  ...
  const getNotificationCounts = () => {
    ...
    if (inboxMessages.length > 0) {
      console.log('Inbox message statuses:', inboxMessages.map(m => m.status));
    }
    if (sentMessages.length > 0) {
      console.log('Sent message read_at values:', sentMessages.map(m => m.read_at));
    }
    if (draftMessages.length > 0) {
      console.log('Draft message statuses:', draftMessages.map(m => m.status));
    }
  ...
  // Temporary test - force some counts to see if UI works
  const testCounts = {
    inbox: 2,
    sent: 1,
    drafts: 0
  ...
  console.log('=== DEBUG: Using test counts for UI testing ===');
  console.log('Real counts:', notificationCounts);
              ...
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center space-x-2">
                  <h4 className={`text-sm font-medium truncate ${
                      ...
                      {(() => {
                        if (activeTab === 'inbox') {
                          const name = senderNames[message.sender_id] || (message.sender_type === 'staff' ? 'Staff Member' : 'Patient');
                          console.log('Inbox - Sender name:', name, 'for ID:', message.sender_id, 'senderNames:', senderNames);
                          return name;
                        } else {
                          const name = recipientNames[message.recipient_id] || (message.recipient_type === 'staff' ? 'Staff Member' : 'Patient');
                          console.log('Sent - Recipient name:', name, 'for ID:', message.recipient_id, 'recipientNames:', recipientNames);
                          return name;
                        }
                      })()}
                  </h4>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-gray-500">
                    {activeTab === 'inbox' ? message.timestamp : new Date(message.sent_at).toLocaleDateString()}
                  </span>
                </div>
              </div>
              <h5 className={`text-sm truncate ${
                  activeTab === 'inbox' && message.status !== 'read' ? 'text-gray-900 font-bold' : 'text-gray-600'
              } font-[family-name:var(--font-adlam-display)]`}>
                {message.subject}
              </h5>
              <p className={`text-xs mt-1 line-clamp-2 font-[family-name:var(--font-adlam-display)] ${
                  activeTab === 'inbox' && message.status !== 'read' ? 'text-gray-700 font-medium' : 'text-gray-500'
              }`}>
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 h-full flex flex-col">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="prose max-w-none">
            ...
            <p className="text-gray-700 leading-relaxed mt-4 font-[family-name:var(--font-adlam-display)]">
              Thank you for your continued trust in our care. Please don't hesitate to reach out if you have any questions or concerns.
            </p>
            <p className="text-gray-700 leading-relaxed mt-4 font-[family-name:var(--font-adlam-display)]">
              Best regards,<br />
              {selectedMessage?.sender}
      ...
      <div className="p-6 border-t border-gray-200 bg-white">
        <div className="flex items-start space-x-4">
          <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center">
            <span className="text-white text-sm font-semibold">
              {patientData ? `${patientData.first_name?.charAt(0) || ''}${patientData.last_name?.charAt(0) || ''}` : 'P'}
            </span>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <PatientNavbar activeTab="messages" />
            ...
            <div className="p-4 border-b border-gray-200">
              <nav className="flex space-x-4">
                {[
                  ...
                  console.log(`Tab ${tab.id}: count = ${tab.count}`);
                  return (
                    <button
                      key={tab.id}
                      onClick={() => {
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="patient">Message Patient</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  {isLoadingRecipients ? (
                    ...
                    >
                      <option value="">
                        {newMessageForm.recipientType === 'staff' 
                      ...
                      {availableRecipients.map((recipient) => (
                        <option key={recipient.id} value={recipient.id}>
                          {newMessageForm.recipientType === 'staff' 
                            ? `${recipient.first_name} ${recipient.last_name} - ${recipient.role} (${recipient.department})`
                            : `${recipient.first_name} ${recipient.last_name} - ${recipient.medical_record_number}`
                          }
                        </option>
                      ))}
                    </select>
