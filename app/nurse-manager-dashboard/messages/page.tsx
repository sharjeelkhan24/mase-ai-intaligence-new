...
export default function NurseMessagesPage() {
  ...
  const loadAvailableRecipients = async (recipientType: string) => {
    ...
    try {
      ...
      if (!staffData?.agency_name) {
        console.error('No agency name found for staff:', staffData);
        return;
      }
      ...
      if (recipientType === 'staff') {
        ...
        if (agencyError || !agency?.staff_table_name) {
          ...
          // Fallback: try to construct table name from agency name
          const constructedTableName = `staff_${staffData.agency_name.toLowerCase().replace(/\s+/g, '_')}`;
          console.log('Trying constructed table name:', constructedTableName);
          
          // Try the constructed table name
          const { data: staffList, error: staffError } = await supabase
            .from(constructedTableName)
            .select('id, first_name, last_name, role, department, email')
            .neq('id', staffData.id)
            .order('first_name');

          console.log('Fallback staff query result:', { staffList, staffError });
          
          if (staffError) {
            console.error('Fallback also failed:', staffError);
            return;
          }
          
          recipients = staffList || [];
          console.log('Using fallback staff recipients:', recipients);
        } else {
          ...
          console.log('Staff query result:', { staffList, staffError });
          ...
          recipients = staffList || [];
          console.log('Final staff recipients:', recipients);
        }
      } else {
        ...
        if (agencyError || !agency?.patients_table_name) {
          ...
          // Fallback: try to construct table name from agency name
          const constructedTableName = `patients_${staffData.agency_name.toLowerCase().replace(/\s+/g, '_')}`;
          console.log('Trying constructed table name:', constructedTableName);
          ...
          console.log('Fallback patients query result:', { patientsData, patientsError });
          
          if (patientsError) {
            console.error('Fallback also failed:', patientsError);
            return;
          }
          
          recipients = patientsData || [];
          console.log('Using fallback patient recipients:', recipients);
        } else {
          ...
          recipients = patientsData || [];
          console.log('Final patient recipients:', recipients);
        }
      }
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 h-full flex flex-col">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="prose max-w-none">
            ...
            <p className="text-gray-700 leading-relaxed mt-4 font-[family-name:var(--font-adlam-display)]">
              Please review this information and let me know if you need any clarification or have questions about the next steps.
            </p>
            <p className="text-gray-700 leading-relaxed mt-4 font-[family-name:var(--font-adlam-display)]">
              Best regards,<br />
              {selectedMessage?.sender}
      ...
      <div className="p-6 border-t border-gray-200 bg-white">
        <div className="flex items-start space-x-4">
          <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center">
            <span className="text-white text-sm font-semibold">
              {staffData ? `${staffData.first_name?.charAt(0) || ''}${staffData.last_name?.charAt(0) || ''}` : 'NM'}
            </span>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <NurseManagerNavbar activeTab="messages" />
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 font-[family-name:var(--font-adlam-display)]">
                Manage communication with nursing staff
              </p>
            </div>
