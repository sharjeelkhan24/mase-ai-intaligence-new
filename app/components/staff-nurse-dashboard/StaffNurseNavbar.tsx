...
interface StaffNurseNavbarProps {
  activeTab: string;
}

export default function StaffNurseNavbar({ activeTab }: StaffNurseNavbarProps) {
  const router = useRouter();
  const { staffData, isLoading } = useStaffData();
  const [agencyName, setAgencyName] = useState('');
  const [userInitials, setUserInitials] = useState('SN');
  const [userDisplayName, setUserDisplayName] = useState('Staff Nurse');
      ...
      if (cachedData) {
        try {
          ...
          if (parsedData?.first_name && parsedData?.last_name) {
            ...
            setUserDisplayName(`${parsedData.first_name} ${parsedData.last_name}`);
          }
        } catch (e) {}
      }
  ...
  // Get user display name from localStorage first to avoid loading state
  ...
  const navigationItems = [
    { id: 'dashboard', label: 'Dashboard', icon: Calendar, href: '/staff-nurse-dashboard' },
    { id: 'schedule', label: 'Schedule', icon: Calendar, href: '/staff-nurse-dashboard/schedule' },
    { id: 'patients', label: 'Patients', icon: Users, href: '/staff-nurse-dashboard/patients' },
    { id: 'self-evaluation', label: 'Self-Evaluation', icon: FileText, href: '/staff-nurse-dashboard/self-evaluation' },
    { id: 'messages', label: 'Messages', icon: MessageSquare, href: '/staff-nurse-dashboard/messages' },
    { id: 'settings', label: 'Settings', icon: Settings, href: '/staff-nurse-dashboard/settings' },
  ];
