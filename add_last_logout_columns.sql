-- =====================================================
-- ADD LAST_LOGOUT COLUMN TO STAFF AND PATIENT TABLES
-- =====================================================
-- This script adds the last_logout column to track when users logged out

-- =====================================================
-- STEP 1: ADD LAST_LOGOUT COLUMN TO ALL STAFF TABLES
-- =====================================================

-- Function to add last_logout column to all staff tables
CREATE OR REPLACE FUNCTION add_last_logout_to_staff_tables()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  LOOP
    ...
    alter_sql := format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS last_logout timestamp without time zone null;', table_record.table_name);
    EXECUTE alter_sql;
    
    -- Create index for last_logout if it doesn't exist
    alter_sql := format('CREATE INDEX IF NOT EXISTS idx_%I_last_logout ON public.%I USING btree (last_logout) TABLESPACE pg_default;', table_record.table_name, table_record.table_name);
    EXECUTE alter_sql;
    
    RAISE NOTICE 'Updated staff table: %', table_record.table_name;
  END LOOP;
END;
$$;

-- Execute the function to update all existing staff tables
SELECT add_last_logout_to_staff_tables();

-- =====================================================
-- STEP 2: ADD LAST_LOGOUT COLUMN TO ALL PATIENT TABLES
-- =====================================================

-- Function to add last_logout column to all patient tables
CREATE OR REPLACE FUNCTION add_last_logout_to_patient_tables()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Updated patient table: %', table_record.table_name;
  END LOOP;
END;
$$;

-- Execute the function to update all existing patient tables
SELECT add_last_logout_to_patient_tables();

-- =====================================================
-- STEP 3: UPDATE TABLE CREATION FUNCTIONS
-- =====================================================

-- Update the create_agency_staff_table function to include last_logout column
...
DECLARE
  ...
  index_sql text;
BEGIN
  -- Create table SQL with last_logout column
  create_sql := format('
  ...
  -- Create indexes SQL including last_logout index
  index_sql := format('
    ...
    CREATE INDEX IF NOT EXISTS idx_%I_last_logout ON public.%I USING btree (last_logout) TABLESPACE pg_default;
...
-- Update the create_agency_patients_table function to include last_logout column
...
GRANT EXECUTE ON FUNCTION add_last_logout_to_staff_tables() TO authenticated;
GRANT EXECUTE ON FUNCTION add_last_logout_to_patient_tables() TO authenticated;
...
-- All staff and patient tables now have the last_logout column and the functions have been updated
-- for future table creation. The last_logout column will track when users logged out from their
-- respective dashboards, providing accurate time information for offline status display.
